...
public class NN{
	...
	public NN(DataSet set, int outputFlag, int threshhold)
	{
		try {
			bw = new BufferedWriter(new FileWriter("NN_Output", false));
		} catch (IOException e) {
...
//		reachabilityDensity = new double[dataSet.size()];
		...
		this.kdistanceCalculation(2);
//		this.findkDistanceNeighborhood();
//		this.computeReachability();
//		this.computeLOF();
		
		NNComparator nncomparator = new NNComparator();
		Collections.sort(dataSet, nncomparator);
		...
		Log("NN\tSrc IP\t\t\tTimestamp");

		for(int i=0;i<dataSet.size();i++)
		{
			if(dataSet.get(i).kdistance>threshhold && (outputFlag == Defines.OUTLIERS||outputFlag == Defines.ALL))
			{
				...
				Log(dataSet.get(i).print(Defines.NN));
			}
			if(dataSet.get(i).kdistance<threshhold && (outputFlag == Defines.NORMAL||outputFlag == Defines.ALL))
			{
			...
			if(dataSet.get(i).kdistance<threshhold && (outputFlag == Defines.OUTLIERS))
			{
				break;
			}
		}
	 ...
	 */
	public void kdistanceCalculation(int k)
	{
...
//			Double kd = new Double(Math.max(firstMin, secondMin));
			Double kd = new Double(distances[i][k-1]);
